...
Points are important information about this data. Points are marked by wine specialist, which make it reliable. The owner only public data which have at least 80 point (out of 100) which means the data contain only good and excellent wine.
Here is the distribution of the point
```{r, include=TRUE}
boxplot(wine$points, main="Boxplot of points")
#bar plot
```
and some plots about other informations:...
...
${|d \in D : t \in d|}$ number of documents where the term t appers. If this value equal 0, it will be adjust to 1
...
+SVM: stand for support vector machine. In the previous kernel, we have vector X for each document. immage that each document is represent by a vector in a multi-dimension space. (The number of dimension is the number of features). The aim is finding a hyper-plane to seperate the space into two class.
++SVM linear and RBF kenels: SVM Linear will decide the hyper-plane as a flat. SVM RBF will use an other dimension to sole the problem. Let's see an example of how linear and RBF kerels work in some pictures:
...
As we can see in the result, The accuracy when training data for NB, SVM Linear and SVM RBF is aa%, bb%, and 81.11%. As a result, I expected that the accuracy when testing with test data will be similar. But it just 74.68% for NB, 78.59% for SVM Linear and 71.36%. SVM Linear is the kernel which has the highest value of accuracy for testing. No information rate (NIR) 0.5323 mean that a class takes 53.23%(Good class), which mean the data is balanced. We can judge that the model is actually worked.
...
We can see that accuracy is slightly increasing (just 0.3%). FN is slightly decreasing. Although the number of FP is increasing but overall, I conclude that the model is better. 

Now double the point only for adjectives:
...
The accuracy is just increasing 0.2%. But the FN is decreasing 29 units, quite impressive. TP for the excellent class is also higher. Although FN is also higher. **I conclude that the best model for "good" - "excellent" wine classify is SVM linear with double tf-idf point for adjectives.**
...
adj_full <- colnames(wine_train_set)[adj_id]
...
wine_train_set[,column_id_full] <- wine_train_set[,column_id_full]*2
wine_test_set[,column_id_full] <- wine_test_set[,column_id_full]*2
...
```{r, eval=FALSE}
train_nb_model2 <- train(x= wine_train_set, y=train$benefit , method = 'naive_bayes')
train_nb_model2
model_nb_result2 <- predict(train_nb_model2, newdata = wine_test_set)

conf_nb_train2 <- table(model_nb_result2, wine_testing_result)
names(dimnames(conf_nb_train2)) <- c("Predicted class", "Actual class")
confusionMatrix(conf_nb_train2)
```

```{r, eval=FALSE}
train_svmLinear_model2 <- train(x= wine_train_set, y=train$benefit , method = 'svmLinear3')
train_svmLinear_model2

```

```{r, eval=FALSE}
model_svmLinear_result2 <- predict(train_svmLinear_model2, newdata = wine_test_set)

conf_svmLinear_train2 <- table(model_svmLinear_result2, wine_testing_result)
names(dimnames(conf_svmLinear_train2)) <- c("Predicted class", "Actual class")
confusionMatrix(conf_svmLinear_train2)
...
wine_testing_result_full <- test_full$benefit

train_svmLinear_model_final2 <- train(x= wine_train_set_full, y=train_full$benefit , method = 'svmLinear3')

model_svmLinear_result_final2 <- predict(train_svmLinear_model_final2, newdata = wine_test_set_full)

conf_svmLinear_train_final2 <- table(model_svmLinear_result_final2, wine_testing_result_full)
names(dimnames(conf_svmLinear_train_final2)) <- c("Predicted class", "Actual class")
...
```{r, include=TRUE}
train_svmLinear_model_final2
confusionMatrix(conf_svmLinear_train_final2)
```


# Futher discusssion
##Suggession for choosing wine
In this section, I'll extract the top 20 influences of each model. 
Here is the result from the SVM Linear for "good" - "excellent" wine model. 
```{r, include=TRUE}
varImp(train_svmLinear_model_final1)
```
Suggesstion for choosing wine:...

Here is the top influences from the SVM Linear for "good" - "excellent" wine model.

```{r, include=TRUE}
varImp(train_svmLinear_model_final2)
```
...
# Conclusion
The issues raised at the beginning are solved with a good result. We figure out that the description and other information printed in a bottle can bring us some ideal about how good the bottle is or how high level of benefit it has. We also have some idea about how can we choose a bottle. In the NLP and ML aspect. SVM linear with double tf-idf point for the adjective is the best model for predicting the quality with aa% accuracy and bb% for FN. For predicting the benefit of a bottle, SVM linear is an acceptable model, with accuracy aa% and FN cc%
...
# Apendix
This report is writen ussing Rmarkdown and adjust by word document. Here is the code used in this project.
